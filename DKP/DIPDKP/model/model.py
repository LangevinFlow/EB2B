...
from .kernel_generate import (gen_kernel_random, gen_kernel_random_motion, make_gradient_filter, 
                            ekp_kernel_generator, get_resnet_kernel, extract_kernel_from_resnet,
...
class DIPDKP:
    ...
    def MCMC_sampling(self):
        if self.kernel_source == 'resnet':
            # Use the pre-loaded ResNet model
            with torch.no_grad():
                kernel_random = extract_kernel_from_resnet(self.resnet_model, self.lr, tuple(self.k_size))
        else:
            # Original MCMC sampling
            if self.conf.model == 'DIPDKP':
                kernel_random = gen_kernel_random(self.k_size, self.sf, self.min_var, self.max_var, 0, 
                                                self.conf.kernel_x, self.conf.kernel_y)
    ...
    def MC_warm_up(self):
        if (self.conf.model == 'DIPDKP' or self.conf.model == 'DIPDKP-motion' or self.conf.model == 'DIPDKP-random-motion'): # and iteration == 0
            ...
            for i in range(self.conf.kernel_first_iteration):
                kernel = self.net_kp(self.kernel_code).clone().view(1, 1, self.kernel_size, self.kernel_size)
                ...
                # Clear gradients after optimization step  
                lossk = lossk.detach()
                self.optimizer_kp.zero_grad()

    def print_and_output_setting(self):
        # Create CSV file and write headers
        csv_path = os.path.abspath(os.path.join(self.conf.output_dir_path, self.conf.img_name + '.csv'))
        with open(csv_path, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['Iteration', 'Image PSNR', 'RE Loss', 'Kernel PSNR'])
            # Write empty rows for future data
    ...
    def print_and_output(self, sr, kernel, kernel_gt, loss_x, i_p):
        ...
        row_idx = (self.iteration * self.conf.I_loop_x + i_p) + 1
        if row_idx < len(rows):
            rows[row_idx] = [row_idx, f'{image_psnr:.2f}', f'{loss_x.data:.2f}', f'{kernel_psnr:.2f}']
    ...
    def __init__(self, conf, lr, hr, device=torch.device('cuda')):
        ...
        # Add kernel source configuration
        self.kernel_source = conf.kernel_source if hasattr(conf, 'kernel_source') else 'mcmc'

        # Load ResNet model if kernel_source is 'resnet'
        self.resnet_model = None
        if self.conf.kernel_source == 'resnet':
            self.resnet_model = models.resnet18(pretrained=True)
            self.resnet_model = self.resnet_model.to(device)
            self.resnet_model.eval()
            
        # Initialize Empirical Bayes kernel estimator if using EB
        self.eb_kernel = None
        if self.conf.kernel_source == 'eb':
            self.eb_kernel = EmpiricalBayesKernel(self.kernel_size, self.sf, device)
    ...
    def train(self):
        ...
        if self.kernel_source == 'mcmc':
            self.MC_warm_up()
            print("-------------------------------- MCMC warm up complete --------------------------------")
        ...
        for self.iteration in tqdm.tqdm(range(self.conf.max_iters), ncols=60):
            ...
            if self.conf.model == 'DIPDKP':
                ...
                # Initialize im_HR_est with the current SR estimate
                self.im_HR_est = sr
                
                # Generate kernel based on the selected method
                if self.kernel_source == 'eb':
                    kernel = self.eb_kernel.optimize(self.im_HR_est, self.lr, current_iter=self.iteration)
                    kernel.requires_grad = True  # Ensure kernel has gradients
                else:
                    # Original MCMC-based kernel generation
                    ...
                    for k_p in range(self.conf.D_loop):
                        kernel_temp = self.net_kp(self.kernel_code).clone().view(1, 1, self.kernel_size, self.kernel_size)
                        self.MCMC_sampling()
                        k_losses[k_p] = self.mse(self.kernel_random, kernel_temp)
                    ...
                    # Generate final kernel for this iteration after optimization
                    kernel = self.net_kp(self.kernel_code).clone().view(1, 1, self.kernel_size, self.kernel_size)

                ac_loss_k = torch.tensor(0.0, device=sr.device, requires_grad=True)
                for i_p in range(self.conf.I_loop_x):
                    # Generate fresh kernel for each inner loop iteration to avoid stale references
                    if self.kernel_source == 'eb':
                        current_kernel = kernel.clone().detach().requires_grad_(True)
                    else:
                        current_kernel = self.net_kp(self.kernel_code).clone().view(1, 1, self.kernel_size, self.kernel_size)
                    ...
                    # Generate fresh kernel for loss_k computation
                    if self.kernel_source == 'eb':
                        kernel_for_loss = kernel.clone().detach().requires_grad_(True)
                    else:
                        kernel_for_loss = self.net_kp(self.kernel_code).clone().view(1, 1, self.kernel_size, self.kernel_size)
                    ...
                    if (self.iteration * self.conf.I_loop_x + i_p + 1) % (self.conf.I_loop_k) == 0:
                        if ac_loss_k.requires_grad:  # Only backward if loss has gradients
                            ac_loss_k.backward(retain_graph=True)
                            self.optimizer_kp.step()
                        ac_loss_k = torch.tensor(0.0, device=ac_loss_k.device, requires_grad=True)

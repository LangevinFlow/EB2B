...
def parameters_setting(conf, args, I_loop_x, I_loop_k, D_loop, method, filename, now_time):
    ...
    # iteration setting
    max_iterations = args.max_iters // I_loop_x if hasattr(args, 'max_iters') else 1000 // I_loop_x       # if DIPDKP->800/1000  if Non-Gaussian Kernel ->400
    ...
    if not hasattr(conf, 'kernel_source'):
        conf.kernel_source = args.kernel_source if hasattr(args, 'kernel_source') else 'mcmc'

    # Empirical Bayes settings
    conf.eb_lr = args.eb_lr if hasattr(args, 'eb_lr') else 1e-3  # Learning rate for Empirical Bayes optimization
    conf.eb_steps = args.eb_steps if hasattr(args, 'eb_steps') else 100  # Number of optimization steps for Empirical Bayes
    conf.eb_prior_weight = args.eb_prior_weight if hasattr(args, 'eb_prior_weight') else 0.1  # Weight for the prior term in Empirical Bayes
